...
export default function Edit({loggedState}) {
    ...
    const socket = io(apiUrl, {
        ...
        reconnectionDelay: 1000,       // Set connection timeout to 10 seconds
    });
    // UseEffect for listening to socket progress updates
    useEffect(()=>{
      ...
      // Handle case when server goes down during persisten connection
      ...
      e.preventDefault();
        if(!inProgress && selectedVideo)
        {
          ...
          fetch(`${apiUrl}/videos/process`, {
            ...
            body: JSON.stringify({options : formData, videoName : selectedVideo.filename, socketId : socket.id, sharpenEnabled : sharpenEnabled}),
            })
            .then((response) => {
              if(!response.ok){
