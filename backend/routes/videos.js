...
router.post('/process', authorisation, async function(req, res, next){
    ...
    try {
        ...
        const result = await executeFFmpegCommand(socketId, req, taskSpec);
        await await req.db.from('journal').select('*').where("filename", outputKey).del();
        return res.status(200).json({ presignedUrl: result.presignedUrl, filename: result.outputKey });
    } catch (error) {
        return res.status(500).json({ error: true, message: error.message });
    }
});

// Helper function for executing ffmpeg process
async function executeFFmpegCommand(socketId, req, taskSpec) {
